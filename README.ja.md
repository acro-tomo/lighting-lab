...
自分の間取りで住宅照明の配置と夜の雰囲気を比較する、ブラウザベースのLDK照明シミュレーターです。
...
非公開のフィードバック専用リポジトリにIssueとして起票されます。トークンと送信先は
サーバ側のSecretとして保持し、公開ソースリポジトリには送信しません。投稿者のGitHubアカウントも不要です。セットアップ手順は
...
プロジェクトデータはブラウザのIndexedDBへ自動保存され、明示的なJSON/PNG書き出しやフィードバック送信以外ではアップロードしません。フィードバックには個人情報や機密を書かないでください。

## ライセンス

[MIT License](LICENSE) — Copyright (c) 2026 Tomoharu Hoshi.
