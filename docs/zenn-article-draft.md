...
title: ブラウザだけで動く「住宅照明シミュレーター」を作った — three-gpu-pathtracerでWYSIWYGパストレーシング
emoji: 💡
type: tech
...
新築やリノベーションでは施工前に照明を決める必要がありますが、図面のダウンライト記号だけでは、照明の違いによる部屋の雰囲気を想像しづらいものです。そこで、間取り図（PDF/画像）を読み込み、壁と照明を配置して、照明の位置・明るさ・色温度と部屋の雰囲気を比較できる無料ブラウザツールを作りました。
