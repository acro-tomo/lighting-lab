...
**非公開のフィードバック専用リポジトリ**にIssueとして自動起票する仕組みのセットアップ手順です。
...
- **Repository access**: Only select repositories → フィードバック専用の非公開リポジトリのみ
...
- 名前: `GITHUB_REPO`
- 値: フィードバック専用の非公開リポジトリ名（`owner/repo`形式）
- 種別: **Secret（暗号化）**
- Production と Preview の両方に設定する
...
```bash
npx wrangler pages secret put GITHUB_TOKEN --project-name lighting-lab
npx wrangler pages secret put GITHUB_REPO --project-name lighting-lab
```

`GITHUB_REPO` は必須です。未設定の場合、Functionは送信を拒否します。公開リポジトリ
`acro-tomo/lighting-lab` は送信先に指定しないでください。フィードバック本文や任意の連絡先が
公開Issueになることを防ぐためです。
