...
1. ~~`text.silent = true` のショット~~ → **不要になった。実装しない。**
   冒頭を無文字にする規約は 2026-08-18 に撤回した（下の「冒頭3秒の型」）。
   全ショットに0.0秒からテロップを出すので、silent フラグは要らない。
...
`REEL_SMOKE=1` で1本流して overlay PNG が意図どおり1行になることを確認する。

## 冒頭3秒の型（最優先。他のどの指示より優先する）

**3秒以内に「何の動画か」が伝わらないものは作らない。**
第4弾の「冒頭2.0秒は無文字」は撤回した。根拠は
marketing/instagram/reels/AUDIENCE-AND-FORMAT.md「冒頭3秒の規約」。
...
新規実装は不要。上下分割は撮影側の `stacked-light-compare`
（capture-decision-reel.mjs:354-356, 457）、分割線は `.split-rule`
（encode-reel.mjs:144、top: 959px）、上下ラベルは
`text.visual.kind = "split"` の `topLabel` / `bottomLabel`（:78-81）。

**各案の s1 は必ずこの表に従う。**各ブロックの s1 行は更新済みなので、
そこに書かれた 0.0秒のテロップをそのまま使う。無文字で始めない。
```
...
| id | 秒 | 内容 | テロップ |
|---|---|---|---|
| e1-dark  | 2.0 | **1灯が点いた状態から開始**→2灯目。カメラ微速前進 | 今、1灯です |
| e2-three | 1.6 | 3灯目 | 3灯 |
| e3-five  | 1.6 | 5灯目 | 5灯 |
| e4-seven | 1.6 | 7灯目 | 7灯 |
...
| id | 秒 | 画 | テロップ |
|---|---|---|---|
| g1-empty | 2.0 | **上下分割で家具あり／なしを同時提示。**0.5秒で割れる | 同じ照明、同じ部屋 |
...
| id | 秒 | 画 | テロップ |
|---|---|---|---|
| h1-swap | 2.0 | **上下分割で広配光／集光を同時提示。**0.5秒で割れる | 穴の位置は同じです |
...
| id | 秒 | 画 | テロップ |
|---|---|---|---|
| s1-dark | 2.0 | 階段が見える引きから（全消灯で始めない）。ゆっくり寄る | 夜中の階段です |
