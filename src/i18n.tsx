...
const en: Record<string, string> = {
  ...
  ,"図面合わせ": "Align plan"
  ,"縮尺を合わせる": "Set scale"
  ,"位置を合わせる": "Set position"
  ,"1階は縮尺合わせで位置が決まります": "On floor 1, the position is set by scale calibration."
  ...
  ,"画面を指でタップ、またはドラッグして、間取り図の基準となる1点目に印を置いてください。位置は指の動きにリアルタイムに追従します。拡大縮小は二本指ピンチで行えます。": "Tap or drag on the screen to place the first reference point on the floor plan. The marker follows your finger in real time. Use a two-finger pinch to zoom in and out."
  ,"同じように2点目を配置してください。確定済みの1点目（緑）からの距離がリアルタイムに表示されます。": "Place the second point the same way. The distance from the confirmed first point (green) updates in real time."
  ,"実距離(mm)を入力し、「確定」を押してください。やり直す場合は「戻る」または「点をやり直す」を使ってください。": "Enter the real-world distance (mm) and press \"Confirm\". Use \"Back\" or \"Redo points\" to start over."
  ,"間取り図": "Floor plan"
  ,"実距離": "Real distance"
  ,"画像倍率": "Image zoom"
  ,"この位置でよければ「1点目を確定」を押してください": "If this position looks right, press \"Confirm point 1\""
  ,"1点目をタップして配置してください": "Tap to place the first point"
  ,"{px}px（この位置でよければ「2点目を確定」を押してください）": "{px}px (if this position looks right, press \"Confirm point 2\")"
  ,"2点目をタップして配置してください": "Tap to place the second point"
  ,"{px}px": "{px}px"
  ,"画像表示をリセット": "Reset image view"
  ,"点をやり直す": "Redo points"
  ,"1点目を確定": "Confirm point 1"
  ,"2点目を確定": "Confirm point 2"
  ,"リセット": "Reset"
  ,"キャンセル": "Cancel"
  ,"確定": "Confirm"
  ,"待機中": "Waiting"
  ...
  ,"前回のプロジェクト「{projectName}」を復元しました。": "Restored your previous project “{projectName}”."
  ...
  ,"IES配光": "IES photometry"
  ,"IES適用中": "IES applied"
  ,"光束（IES由来）": "Luminous flux (from IES)"
  ,"ビーム角（IES由来）": "Beam angle (from IES)"
  ,"色温度・調光（設定のまま）": "Color temperature / dimming (your settings)"
  ,"照度計算・3D描画ともIESのθ/φ配光を使います。": "Both illuminance and the 3D view use the full θ/φ IES distribution."
  ,"照度計算はIESのθ/φ配光をそのまま使います。3D描画はφ=0断面での近似です。": "Illuminance uses the full θ/φ IES distribution. The 3D view approximates it with the φ=0 slice."
  ,"配光の向き": "Distribution orientation"
  ,"非対称配光です。向きを回すと明るい側が動きます。": "This is an asymmetric distribution. Rotating it moves the bright side."
  ,"IESファイルがありません／再選択が必要": "IES file not found — please select it again"
  ,"IESファイルを読み込み中…": "Loading IES file…"
  ,"IESを解析中…": "Parsing IES…"
  ...
  ,"IESはこのブラウザ内でのみ解析・保存します。サーバーへは送信しません。": "IES files are parsed and stored only in this browser. They are never sent to a server."
  ...
  ,"1階基準で仮合わせしたので、図面合わせ →「位置を合わせる」で位置を確認してください。": "Using temporary floor 1 alignment. Check the position with Align plan > Set position."
  ...
  ,"プロジェクト「{projectName}」を読み込みました。": "Loaded project “{projectName}”."
  ,"プロジェクト名を入力してください。": "Enter a project name."
  ,"プロジェクト「{projectName}」を保存しました。": "Saved project “{projectName}”."
